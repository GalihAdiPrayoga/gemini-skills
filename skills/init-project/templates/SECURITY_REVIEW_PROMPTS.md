# Security & Review Prompts — {{PROJECT_NAME}}
...
## 8. MCP / Plugin Audit
> Audit active MCP servers against `SECURITY.md` §4. List each tool's permissions; confirm least-privilege: no production secrets, no prod DB, no auto-deploy, no unrestricted shell/filesystem. Flag anything exceeding the Allowed list; recommend tightening or removal.
