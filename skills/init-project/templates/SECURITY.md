...
Security policy for human + AI contributors. Companion to `AGENTS.md` (checklist) and `CLAUDE.md` (rules). When this conflicts with a quick fix, **security wins** — stop and resolve.
...
- Client-exposed values use the public env prefix; secrets never do. Rotate any suspected-exposed credential immediately (§7).
...
**Allowed:** read/write project folder · run lint/typecheck/build/test · run gitleaks/semgrep/trivy · GitHub repo (read/PR) · staging URL · staging DB **read-only**.
**Blocked:** read production secrets · SSH prod / unrestricted shell · write/migrate production DB · auto-deploy / DNS / secret management · delete database/storage · unrestricted filesystem access.

## 5. Required Security Scan Commands
```bash
{{PKG_MANAGER}} run security:secrets   # gitleaks — secret scan
{{PKG_MANAGER}} run security:code      # semgrep --config auto — SAST
{{PKG_MANAGER}} run security:fs        # trivy fs — deps/secret/misconfig
{{PKG_MANAGER}} run security:all       # all of the above
```
> gitleaks, semgrep, trivy are external CLIs — install separately (see `docs/MCP_RECOMMENDATIONS.md`).

## 6. Pre-Deploy Checklist
- [ ] lint + typecheck clean · build passes
...
## 7. Incident Response — Secret Leak
1. **Rotate first** at the provider — rotation > cleanup.
...
5. **Record** a short dated note (what leaked, blast radius, rotation done) — never the secret value.
