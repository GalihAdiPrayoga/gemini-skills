...
tools: [read_file, replace, write_file, search_file_content, glob]
---
...
No `run_shell_command` available — cannot shell out, cannot push, cannot delete.

## Workflow

1. `read_file` target(s). Never edit blind.
2. `replace` smallest diff that work.
3. Re-`read_file` to verify.
4. Return receipt.
